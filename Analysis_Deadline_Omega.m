...
max_Omega = 1.12; % max redundancy ratio
...
deadline = 3;
Omega_vec = 1:0.005:max_Omega;

failure = zeros (1,J,length(Omega_vec));
failure_layer = zeros (L,J,length(Omega_vec));
...
s = {};
hold on
% pp = plot(Omega_vec,1-sum(reshape(failure,J,length(Omega_vec)))/J ,'k-' , 'linewidth',1.5 );
% p = [p,pp];
% s = [s,'No layering'];
cc = hsv(L); % creates colormap
cc(2,:)=[0,0.5,0];
for l=1:L
    pp = plot(Omega_vec,1-sum(reshape(failure_layer(l,:,:),...
        J,length(Omega_vec)))/J , 'color',cc(l,:),'linewidth',1.5 );
    p = [p,pp];
...
h.Position(2) = .7 - h.Position(4)/2;
grid on
xlabel('Redundancy ratio','fontsize',22);
ylabel('Success rate','fontsize',22);
xlim([1,max_Omega])
